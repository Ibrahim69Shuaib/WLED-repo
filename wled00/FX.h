...
private:
  ...
  struct Transition
  {
    ...
    Transition(uint16_t dur = 750)
        : _briT(255), _cctT(127), _palT(CRGBPalette16(CRGB::Black)), _prevPaletteBlends(0), _modeP(FX_MODE_STATIC), _start(millis()), _dur(dur)
    {
    }
    Transition(uint16_t d, uint8_t b, uint8_t c, const uint32_t *o)
        : _briT(b), _cctT(c), _palT(CRGBPalette16(CRGB::Black)), _prevPaletteBlends(0), _modeP(FX_MODE_STATIC), _start(millis()), _dur(d)
    {
...
public:
  ...
  ~Segment()
  {
    ...
    if (canUseSerial())
    {
      ...
      if (name)
        Serial.printf(" name=%s (%p)", name, name);
      if (data)
        Serial.printf(" dataLen=%d (%p)", (int)_dataLen, data);
      if (ledsrgb)
        Serial.printf(" [%sledsrgb %u bytes]", Segment::_globalLeds ? "global " : "", length() * sizeof(CRGB));
      if (strip_uses_global_leds() == true)
        Serial.println((Segment::_globalLeds != nullptr) ? F(" using global buffer.") : F(", using global buffer but Segment::_globalLeds is NULL!!"));
      Serial.println();
...
// WLEDMM only delete segments when they are not in use
    ...
    if ((Segment::_globalLeds == nullptr) && !strip_uses_global_leds() && (ledsrgb != nullptr))
    {
      free(ledsrgb);
      ledsrgb = nullptr;
    } // WLEDMM we need "!strip_uses_global_leds()" to avoid crashes (#104)
    if (name)
    {
      delete[] name;
      name = nullptr;
  ...
  inline uint8_t currentBri(uint8_t briNew, bool useCct = false)
  {
    ...
    {
      if (useCct)
        return ((briNew * prog) + _t->_cctT * (0xFFFFU - prog)) >> 16;
      else
        return ((briNew * prog) + _t->_briT * (0xFFFFU - prog)) >> 16;
    }
    else
    {
  ...
  { // blur all columns
    ...
    for (unsigned k = 0; k < cols; k++)
      blurCol(k, blur_amount, smear);
  }
  inline void blurRows(fract8 blur_amount, bool smear = false)
  { // blur all rows
    const unsigned rows = virtualHeight();
    for (unsigned i = 0; i < rows; i++)
      blurRow(i, blur_amount, smear);
  }

  // 2D matrix
#ifndef WLEDMM_FASTPATH
  inline uint16_t virtualWidth() const
  { // WLEDMM use fast types, and make function inline
  ...
  inline uint16_t virtualHeight() const
  { // WLEDMM use fast types, and make function inline
    uint_fast16_t groupLen = groupLength();
  ...
  inline uint16_t XY(uint_fast16_t x, uint_fast16_t y) const
  {                                                           // support function to get relative index within segment (for leds[]) // WLEDMM inline for speed
  ...
  inline void setPixelColorXY(int x, int y, uint32_t col)
  {
    ...
    { // fast path
      ...
      if (!_isValid2D)
        return; // not active
      if ((unsigned(x) >= _2dWidth) || (unsigned(y) >= _2dHeight))
        return; // check if (x,y) are out-of-range - due to 2's complement, this also catches negative values
      if (!_brightness && !transitional)
        return; // black-out
  ...
  inline uint32_t getPixelColorXY(int x, int y) const
  {
    ...
    if (!_isValid2D)
      return 0; // not active
    if ((unsigned(x) >= _2dWidth) || (unsigned(y) >= _2dHeight))
      return 0; // check if (x,y) are out-of-range - due to 2's complement, this also catches negative values
    if (ledsrgb)
    {
      ...
      return getPixelColorXY_part2(x, y, int(_2dWidth), int(_2dHeight)); // call "no ledsrgb" function to retrieve pixel from bus driver
  }
#else
...
#ifdef WLED_DEBUG
    if (Serial)
      Serial.println(F("~WS2812FX destroying strip.")); // WLEDMM can't use DEBUG_PRINTLN here
#endif
