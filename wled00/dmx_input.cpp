...
bool DMXInput::installDriver()
{
  ...
  // Initialize DMX in input mode
  DMX::Initialize(input); // 'input' is an enum defined in the library

  USER_PRINTF("Listening for DMX on pin %u\n", rxPin);
  USER_PRINTF("DMX enable pin is: %u\n", enPin);

  // Setup UDP if not already done
  if (!udpSetup)
  {
    dmxUdp.begin(DMX_UDP_PORT);
    udpSetup = true;
  }
...
void DMXInput::updateInternal()
{
  ...
  std::lock_guard<std::mutex> lock(dmxDataLock);

  // Read all DMX channels
  DMX::ReadAll(dmxdata, 1, DMX_PACKET_SIZE - 1); // Skip start code

  // Broadcast over UDP if connected to WiFi
  if (udpSetup && WiFi.status() == WL_CONNECTED)
  {
    IPAddress broadcastIp = ~uint32_t(Network.subnetMask()) | uint32_t(Network.gatewayIP());
    dmxUdp.beginPacket(broadcastIp, DMX_UDP_PORT);
    dmxUdp.write(dmxdata, DMX_PACKET_SIZE);
    dmxUdp.endPacket();
  }
}
...
{
  if (!initialized && rxPin != 255 && txPin != 255)
  {
    installDriver();
  }
}
...
#endif
// // TODO: new code to fix SOC_UART_NUM error
// // FIXME: SOC_UART_NUM is causing compiling errors
// #ifdef ESP32
// #include "soc/uart_reg.h"
// #ifndef SOC_UART_NUM
// #define SOC_UART_NUM 3 // Most ESP32 variants have 3 UART ports
// #endif
// #endif
// // TODO: code before this comment is for fixing SOC_UART_NUM error
// #include "wled.h"
// #include <string>
...
// // {
// //   // TODO: modified some lines of the config and removed unsupported codes
// //   dmx_config_t config = {}; // Initialize all members to 0/null
// //   config.pd_size = 255;
...
// //   // Remove or comment out these lines as they're not supported
...
// //   // Instead, use direct string assignment if needed
// //   snprintf(config.software_version_label, sizeof(config.software_version_label),
// //            "WLED_V%d", VERSION);
...
//   char versionBuffer[33];
//   sprintf(versionBuffer, "WLED_V%d", VERSION);
//   strncpy(config.software_version_label, versionBuffer, 32);
...
//   if (!udpSetup)
//   {
//     udpSetup = dmxUdp.begin(DMX_UDP_PORT);
//     if (!udpSetup)
//     {
//       USER_PRINTLN("Failed to start DMX UDP");
//     }
//   }
// }
...
//     // TODO: New code here
//     //  Broadcast DMX data via UDP
//     if (udpSetup && WLED_CONNECTED)
//     {
//       IPAddress broadcastIp;
//       broadcastIp = ~uint32_t(Network.subnetMask()) | uint32_t(Network.gatewayIP());

//       // Create Art-Net packet
//       uint8_t artnetPacket[18 + DMX_PACKET_SIZE];
//       memcpy(artnetPacket, "Art-Net", 8);
//       artnetPacket[8] = 0x00;                           // OpCode LSB
//       artnetPacket[9] = 0x50;                           // OpCode MSB (OpDmx)
//       artnetPacket[10] = 0x00;                          // Protocol version MSB
//       artnetPacket[11] = 14;                            // Protocol version LSB
//       artnetPacket[12] = 0;                             // Sequence
//       artnetPacket[13] = 0;                             // Physical input port
//       artnetPacket[14] = 0;                             // Universe LSB
//       artnetPacket[15] = 0;                             // Universe MSB
//       artnetPacket[16] = (DMX_PACKET_SIZE >> 8) & 0xFF; // Length MSB
//       artnetPacket[17] = DMX_PACKET_SIZE & 0xFF;        // Length LSB

//       // Copy DMX data
//       memcpy(artnetPacket + 18, dmxdata, DMX_PACKET_SIZE);

//       // Send packet
//       dmxUdp.beginPacket(broadcastIp, DMX_UDP_PORT);
//       dmxUdp.write(artnetPacket, 18 + DMX_PACKET_SIZE);
//       dmxUdp.endPacket();
//     }
//   }
// }
